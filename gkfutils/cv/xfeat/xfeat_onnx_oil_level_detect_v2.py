...
def check_oil_level_percent(lp):
    """
    检查油位百分比是否正确
    """
    if lp is not None:
...
def get_final_results(res, l, lp, res2, l2, lp2):
...
def detect_oil_level_by_xfeat_onnx_v2(ort_session, im1, im2, roi_rect=None, alg=1, oil_color=["yellow", "orange"], oil_range=(115, 325), reduce_sum_thr=500, area_thresh=100, color=(255, 255, 0), thickness=4):
    ...
    if alg == 0:
        ...
        res, l, lp = get_final_results(res, l, lp, res2, l2, lp2)
        canvas = hstack_canvas(canvas, res)
        return canvas, res, l, lp
...
def main_test_20250411(data_path):
    dirname = os.path.basename(data_path)
    save_path = os.path.abspath(os.path.join(data_path, '../{}_xfeat_matching_results'.format(dirname)))
    os.makedirs(save_path, exist_ok=True)
