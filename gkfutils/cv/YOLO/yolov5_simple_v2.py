...
class CSPDarkNet(nn.Module):
    def __init__(self, gd=0.33, gw=0.5):
        ...
        self.small = nn.Sequential(
            Conv(3, 64, k=6, s=2, p=2),
            Conv(64, 128, k=3, s=2),
            C3(128, 128, n=1, shortcut=True, e=gw),
            Conv(128, 256, k=3, s=2),
            C3(256, 256, n=1, shortcut=True, e=gw)
        )
        self.medium = nn.Sequential(
            Conv(256, 512, k=3, s=2),
            C3(512, 512, n=1, shortcut=True, e=gw)
        )
        self.large = nn.Sequential(
            Conv(512, 1024, k=3, s=2),
            C3(1024, 1024, n=1, shortcut=True, e=gw),
            SPPF(1024, 1024, k=5)
        )
...
class yolov5(nn.Module):
    def __init__(self, nc=80, gd=0.33, gw=0.5, clsFlag=True):
        ...
        self.small_1 = nn.Sequential(
            C3(1024, 512, n=1, shortcut=False),
        ...
        self.large_22 = nn.Sequential(
            C3(1024, 1024, n=1, shortcut=False)
        )

        # ------------------------------Prediction--------------------------------
        outInfoNum = 6 if clsFlag else 5
        self.small_out = nn.Sequential(
            Conv(256, 3 * (outInfoNum + nc), 1, 1, 0)
        )
        self.medium_out = nn.Sequential(
            Conv(512, 3 * (outInfoNum + nc), 1, 1, 0)
        )
        self.large_out = nn.Sequential(
            Conv(1024, 3 * (outInfoNum + nc), 1, 1, 0)
        )
...
if __name__ == '__main__':
    ...
    net = yolov5(nc=80, gd=net_size[0], gw=net_size[1], clsFlag=False)
    print(net)
    # a = torch.randn(1, 3, 384, 384)
    # a = torch.randn(1, 3, 416, 416)
    # a = torch.randn(1, 3, 640, 640)
    # a = torch.randn(1, 3, 768, 768)
    a = torch.randn(1, 3, 448, 768)
