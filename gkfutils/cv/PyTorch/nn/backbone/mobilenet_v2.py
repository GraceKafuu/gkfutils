...
class MyMobileNetV2(nn.Module):
    def __init__(
        ...
        self.features2 = self.features[4:7]
        self.features3 = self.features[7:14]
        self.features4 = self.features[14:]
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # This exists since TorchScript doesn't support inheritance, so the superclass method
        # (this one) needs to have a name other than `forward` that can be accessed in a subclass
        x = self.features(x)  # torch.Size([1, 1280, 20, 20])
        # Cannot use "squeeze" as batch-size can be 1
        x = nn.functional.adaptive_avg_pool2d(x, (1, 1))  # torch.Size([1, 1280, 1, 1])
        x = torch.flatten(x, 1)  # torch.Size([1, 1280])
    ...
    def forward(self, x: Tensor) -> Tensor:
        x1 = self.features1(x)   # torch.Size([1, 24, 160, 160])
        x2 = self.features2(x1)  # torch.Size([1, 32, 80, 80])
        x3 = self.features3(x2)  # torch.Size([1, 96, 40, 40])
        x4 = self.features4(x3)  # torch.Size([1, 1280, 20, 20])
        return (x1, x2, x3, x4)
...
if __name__ == '__main__':
    ...
    weights = "F:\\Google_Chrome_Downloads\\mobilenet_v2-b0353104.pth"
